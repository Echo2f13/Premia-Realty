/**
 * Automated Property Seeder (No Interactive Prompts)
 *
 * Usage:
 * 1. Set env vars: ADMIN_EMAIL and ADMIN_PASSWORD in .env
 * 2. Or pass as args: node seedPropertiesAuto.js admin@example.com password123
 * 3. Run: node seedPropertiesAuto.js
 */
...
// Get credentials from env vars or command line
...
// Sample property data (same as seedProperties.js)
...
async function seedProperties() {
  console.log('🌱 Starting Firestore property seeding (automated)...\n');

  if (!ADMIN_EMAIL || !ADMIN_PASSWORD) {
    console.error('❌ Missing credentials!');
    console.error('\nUsage:');
    console.error('  node seedPropertiesAuto.js <email> <password>');
  ...
  try {
    ...
    if (!idTokenResult.claims.admin) {
      ...
      console.error('  cd db && node set-admin.js ' + ADMIN_EMAIL);
      process.exit(1);
    }
